...
session_start();
    ...
    $stok = $_POST['stok_bahan_baku'];
    $debit = $_POST['Debit'];

    foreach($account as $key => $account_v) {
        
        $debit_v = mysqli_real_escape_string($db2,(double)str_replace(",",".",str_replace(".","",$debit[$key])));
        $stok_v = mysqli_real_escape_string($db2,(double)str_replace(",",".",str_replace(".","",$stok[$key])));

        if($debit_v > $stok_v ){

            $stmt1 = $db2->prepare("INSERT INTO `pengambilan_bahan_baku` (id_bahan_baku, nik, jumlah_pengambilan, tanggal_pengambilan) VALUES(?, ?, ?, ?)");
            $stmt1->bind_param("ssss", $account_vX, $id_karyawanX, $debit_v, $tgl_pengambilan);
            ...
            echo "Jumlah tidak bisa lebih besar dari stok";
        }
    }

    header("location:../pengambilan_bb.php");
